...
var JSDoc = inherit({
    ...
    parse : function(source) {
        ...
        if(errors.length) throw Error(errors.join(new Array(4).join(EOL)));
    ...
    _parseJSDocComment : function(comment, errors) {
        ...
        lines.forEach(function(line, i) {
            ...
            if(matches) {
                ...
                tag.comment += (tag.comment? EOL : '') + line.replace(/^\s*\*\s?|\s*$/g, '');
            }
        }, this);
    ...
    _buildError : function(e, source, commentAstNode) {
        ...
        return Error(
            msg + EOL +
            new Array(msg.length + 1).join('-') + EOL +
            source.split(EOL).slice(loc.start.line - 1, loc.end.line + 3).join(EOL) + EOL +
            e.stack);
    }
});
